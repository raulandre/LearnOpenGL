...
vec3 CalcSpotLight(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir) {
    ...
    // combine results
    vec4 texColor1 = texture(material.texture_diffuse, TexCoords);
    vec4 texColor2 = texture(material.texture_specular, TexCoords);

    vec3 ambient = light.ambient * vec3(texColor1);
    vec3 diffuse = light.diffuse * diff * vec3(texColor1);
    vec3 specular = light.specular * spec * vec3(texColor2);
